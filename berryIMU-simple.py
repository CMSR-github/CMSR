...
while True:
    ...
    eps = 1
    if abs(accYnorm - prevY) > eps:
        print(f"MOVED {accXnorm}, {accYnorm}")
    prevX, prevY = accXnorm, accYnorm
    if i % 30 == 0:

        if 1:                       #Change to '0' to stop showing the angles from the accelerometer
            outputString += "#  ACCX Angle %5.2f ACCY Angle %5.2f  # MAG: %5.5f, %5.5f" % (AccXangle, AccYangle, accXnorm, accYnorm)
